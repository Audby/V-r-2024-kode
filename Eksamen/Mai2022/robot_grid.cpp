...
{
    // BEGIN: G1
    double numHorizontalLines = (h_size - y_pos) / cell_height;
    double numVerticalLines = (w_size - x_pos) / cell_width;
    for (int i = 0; i<numHorizontalLines; i++) {
      window.draw_line({x_pos, y_pos * (i+1)}, {cell_width, y_pos * (i+1)});
    }
